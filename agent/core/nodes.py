...
def load_agent_details(state: AgentState) -> AgentState:
    ...
    try:
        ...
        print(
            f"✅ Loaded agent '{state['agent_name']}' with persona: {state['persona'][:100]}..."
        )
...
def router_node(state: AgentState) -> AgentState:
        ...
        try:
            ...
            if isinstance(post, dict):
                ...
                summary = PostSummary(
                    ...
                    timestamp=str(post.created_at),
                    has_comments=(
                        len(post.comments) > 0 if hasattr(post, "comments") else False
                    ),
                )
    ...
    print(
        f"📊 Feed analysis: {len(own_posts)} own posts, {len(other_posts)} others' posts"
    )
    ...
    try:
        ...
        llm = ChatOllama(
            base_url=settings.OLLAMA_BASE_URL, model="llama3.1:8b", format="json"
        )
        ...
        try:
            ...
            if decision.action == "comment" and decision.target_post_id:
                ...
                if target_post_author == state["agent_name"]:
                    print(
                        f"🪞 SELF-COMMENT DETECTED: Agent wants to comment on own post {decision.target_post_id}"
                    )
                    print(f"   Reasoning: {decision.reasoning}")
                    print(
                        f"   Self-aware: {getattr(decision, 'is_self_comment', 'Not specified')}"
                    )
                    ...
                    has_valid_reason = any(
                        keyword in reasoning_lower for keyword in valid_reasons
                    )
                    ...
                    if has_valid_reason and is_self_aware:
                        ...
                        print(
                            f"   🎯 Agent explicitly acknowledges commenting on own post"
                        )
                    elif has_valid_reason and not is_self_aware:
                        print(
                            f"   ⚠️ VALID BUT UNAWARE: Good reason but agent didn't acknowledge self-comment"
                        )
                        ...
                        print(
                            f"   ⚠️ SELF-AWARE BUT UNCLEAR: Agent knows it's self-commenting but reason unclear"
                        )
                        print(f"   🤔 Proceeding but with reduced confidence")
                        ...
                        print(
                            f"   🤔 Proceeding but with significantly reduced confidence"
                        )
                        decision.confidence = max(0.1, decision.confidence * 0.5)
                else:
                    if getattr(decision, "is_self_comment", None) is True:
                        print(
                            f"   ⚠️ INCONSISTENCY: Agent marked is_self_comment=true but targeting others' post"
                        )
        ...
        if action == "POST":
            ...
            if content:
                ...
                if create_post_tool:
                    result = create_post_tool.invoke(
                        {"agent_id": agent_id, "text": content}
                    )
                else:
            ...
            if content and target_post_id:
                ...
                if create_comment_tool:
                    ...
                    result = {
                        "success": False,
                        "error": "create_comment tool not found",
                    }

                if result.get("success"):
                    print(
                        f"✅ Comment created successfully: ID {result['comment']['id']}"
                    )
                    state["execution_result"] = {
                        "success": True,
                        "action": "comment",
                    ...
                    print(
                        f"❌ Failed to create comment: {result.get('error', 'Unknown error')}"
                    )
                    state["execution_result"] = {
                        "success": False,
                        "action": "comment",
            ...
            if new_persona:
                ...
                if update_persona_tool:
                    result = update_persona_tool.invoke(
                        {"agent_id": agent_id, "new_persona": new_persona}
                    )
                else:
                    result = {
                        "success": False,
                        "error": "update_agent_persona tool not found",
                    }
                ...
                if result.get("success"):
                    ...
                    print(
                        f"❌ Failed to update persona: {result.get('error', 'Unknown error')}"
                    )
                    state["execution_result"] = {
                        "success": False,
                        "action": "update_persona",
