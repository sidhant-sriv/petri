...
def search_agents_by_persona(search_term: str) -> List[Dict[str, Any]]:
    ...
    return [agent for agent in agents if search_term_lower in agent["persona"].lower()]
...
def get_posts_by_agent(
    agent_id: int, skip: int = 0, limit: int = 50
) -> List[Dict[str, Any]]:
    """
...
def search_posts(search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    search_term_lower = search_term.lower()
    matching_posts = [
        post for post in feed if search_term_lower in post["text"].lower()
    ]
...
def get_filtered_feed(
    ...
    for post in feed:
        ...
        if agent_names:
            if post["author"]["name"].lower() not in [
                name.lower() for name in agent_names
            ]:
                continue

        # Exclude specific agents
        if exclude_agents:
            if post["author"]["name"].lower() in [
                name.lower() for name in exclude_agents
            ]:
                continue
...
def get_world_stats() -> Dict[str, Any]:
    ...
    most_active_agent_id = (
        max(agent_post_counts, key=agent_post_counts.get) if agent_post_counts else None
    )
    most_active_agent = (
        get_agent(most_active_agent_id) if most_active_agent_id else None
    )

    # Find most commented post
    most_commented_post = (
        max(feed, key=lambda p: len(p.get("comments", []))) if feed else None
    )
    ...
    return {
        ...
        "most_active_agent_post_count": (
            agent_post_counts.get(most_active_agent_id, 0)
            if most_active_agent_id
            else 0
        ),
        "most_commented_post": most_commented_post,
        "most_commented_post_count": (
            len(most_commented_post.get("comments", [])) if most_commented_post else 0
        ),
    }
        ...
        for post in feed:
            for comment in post.get("comments", []):
                if search_term.lower() in comment["text"].lower():
                    matching_comments.append({**comment, "parent_post": post})
...
def analyze_conversation_dynamics(agent_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        if agent_id:
            ...
            return {"world_statistics": world_stats, "analysis_type": "general"}
    except WorldAPIError as e:
...
def get_engagement_opportunities(
    min_comments: int = 0, max_age_hours: int = 24
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        active_posts = get_filtered_feed(
            min_comments=min_comments, hours_back=max_age_hours, limit=15
        )
    ...
    try:
        ...
        return {
            ...
            "activity_summary": {
                ...
                "hours_analyzed": hours,
                "last_post_time": (
                    recent_posts[0]["created_at"] if recent_posts else None
                ),
                "last_comment_time": (
                    recent_comments[0]["created_at"] if recent_comments else None
                ),
            },
        }
    except WorldAPIError as e:
...
MEMORY_TOOLS = [find_agents_by_persona, find_agent_by_name, search_conversation_history]
...
def create_post(agent_id: int, text: str) -> Dict[str, Any]:
    ...
    """
    try:
        post_data = {"text": text, "agent_id": agent_id}

        result = _make_request("/api/posts/", method="POST", data=post_data)

        logger.info(f"✅ Created post for agent {agent_id}: {text[:50]}...")
        return {"success": True, "post": result, "message": "Post created successfully"}
    except WorldAPIError as e:
        logger.error(f"❌ Failed to create post: {e}")
        return {"success": False, "error": str(e), "message": "Failed to create post"}
...
def create_comment(agent_id: int, post_id: int, text: str) -> Dict[str, Any]:
    ...
    """
    try:
        comment_data = {"text": text, "agent_id": agent_id}

        result = _make_request(
            f"/api/posts/{post_id}/comments/", method="POST", data=comment_data
        )

        logger.info(
            f"✅ Created comment on post {post_id} for agent {agent_id}: {text[:50]}..."
        )
        return {
            "success": True,
            "comment": result,
...
def update_agent_persona(agent_id: int, new_persona: str) -> Dict[str, Any]:
    ...
    """
    try:
        persona_data = {"persona": new_persona}

        result = _make_request(
            f"/api/agents/{agent_id}/persona", method="PUT", data=persona_data
        )
...
ACTION_TOOLS = [create_post, create_comment, update_agent_persona]
